...
void spot_light_fragment(vec3 normal, vec3 pos, out vec3 totalLightContrib)
{
   ...
   {      
      ...
      //this will be zero if spotEffect is less then spotCosCutoff one otherwise
      float spotToggle = ceil(max(spotEffect, spotCosCutoff) - spotCosCutoff);
      ...
      vec3 dotProductLighting = normalDotLight * spotLights[i+1].xyz;
      ...
      totalLightContrib +=  spotToggle * absIntensity * min(1.0, atten) * dotProductLighting; 
   } 
