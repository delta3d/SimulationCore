...
void main()
{
   ...
   float dist = length(vec3(gl_ModelViewMatrix * gl_Vertex).xyz);

   //Finally, mix the results with the fog amount and compute the gl_Position
   //computeTerrainFog(gl_Fog.end, dist, vFog.x);

   ///compute the amount of underwater fog
   vFog.y = dist;//computeFog(0.0, UnderWaterViewDistance, dist); 
